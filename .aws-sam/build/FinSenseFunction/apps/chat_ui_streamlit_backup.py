...
# =========================
# Sidebar (with stable keys so we can sync from chat)
# =========================
st.sidebar.header("Configuration")
if "age_sb" not in st.session_state: st.session_state.age_sb = 25
if "risk_sb" not in st.session_state: st.session_state.risk_sb = "moderate"
if "horizon_sb" not in st.session_state: st.session_state.horizon_sb = 5
...
risk = st.sidebar.selectbox("Risk Profile", ["conservative", "moderate", "aggressive"],
                            index=["conservative","moderate","aggressive"].index(st.session_state.risk_sb), key="risk_sb")
...
# =========================
# Session state for chat & amount/percent intent
# =========================
...
def parse_overrides(text: str) -> dict:
    """
    Extract age, risk, horizon, amount(£), and percent intent from free text.
    Accepts phrases like:
      - "55 year old", "I'm 55 years old"
      - "high risk", "aggressive" / "safe", "defensive", "low risk", "conservative"
      - "over the next 14 years", "7y", "10 yrs"
      - "£3,000.25", "3000 pounds", "invest 3000", "invest 3000.25"
      - "save 10%", "invest 15 percent"
    """
    text_l = text.lower()
    o: Dict[str, Any] = {}

    # Age: e.g., "55 year old" or "55 years old"
    m_age = re.search(r'(?:^|\D)(1[6-9]|[2-9]\d|100)\s*year(?:s)?\s*old\b', text_l)
    if m_age:
        o["age"] = int(m_age.group(1))
    ...
    # Horizon: "14 years", "7y", "10 yrs"
    m_h = re.search(r'\b(1|[2-3]?\d)\s*(?:y|yr|yrs|year|years)\b', text_l)
    if m_h:
        o["horizon_years"] = int(m_h.group(1))

    # Amount in GBP:
    # - "£3000.25"
    # - "3000 pounds" / "3000 gbp"
    # - bare number near 'invest' like "invest 35401"
    text_norm = text_l.replace(",", "")
    m_amt = re.search(r'£\s*([0-9]+(?:\.[0-9]{1,2})?)', text_norm) \
           or re.search(r'([0-9]+(?:\.[0-9]{1,2})?)\s*(pounds|gbp)\b', text_norm)

    if not m_amt:
        # bare number if directly tied to "invest"
        m_amt = re.search(r'\binvest\s+([0-9]+(?:\.[0-9]{1,2})?)\b', text_norm)

    if m_amt:
        try:
            o["amount_gbp"] = float(m_amt.group(1))
        except Exception:
            pass

    # Percent intent: "save 10%", "invest 15 percent"
    m_pct = re.search(r'(\d{1,2}(?:\.\d+)?)\s*%', text_norm) \
           or re.search(r'(\d{1,2}(?:\.\d+)?)\s*percent', text_norm)
    if m_pct:
        try:
            o["percent_intent"] = float(m_pct.group(1))
        except Exception:
            pass
...
def amount_breakdown_gbp(amount: float, alloc: dict) -> dict:
    return {
        'Equities (£)': round(amount * float(alloc.get('equities', 0)), 2),
        'Bonds (£)':    round(amount * float(alloc.get('bonds', 0)), 2),
        'Cash (£)':     round(amount * float(alloc.get('cash', 0)), 2),
    }
...
def call_api(message: str) -> Dict[str, Any]:
    ...
    if use_sidebar:
        payload["profile"] = {"age": int(st.session_state.age_sb), "risk": st.session_state.risk_sb, "horizon_years": int(st.session_state.horizon_sb)}

    overrides = parse_overrides(message)

    # apply overrides into payload
    if "age" in overrides:
        payload["profile"]["age"] = overrides["age"]
        st.session_state.age_sb = overrides["age"]  # sync sidebar
    if "risk" in overrides:
        payload["profile"]["risk"] = overrides["risk"]
        st.session_state.risk_sb = overrides["risk"]  # sync sidebar
    if "horizon_years" in overrides:
        payload["profile"]["horizon_years"] = overrides["horizon_years"]
        st.session_state.horizon_sb = overrides["horizon_years"]  # sync sidebar

    # store chat-only intents
...
with st.chat_message("assistant"):
    ...
    with col2:
        ...
        # save a succinct text copy into chat history
        summary_text = f"{advice.get('summary','(no summary)')}\n\nAction: {advice.get('one_action','—')}"
        st.session_state.messages.append({"role": "assistant", "content": summary_text})

        # since we synced sidebar state, re-render so the new values appear
        st.rerun()
